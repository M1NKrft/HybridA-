...
class C:  # Parameter config
    ...
    MAX_STEER = 0.006# [rad] maximum steering angle
...
def generate_track(csv_file: str, track_width: float):
    ...
    t_fine = np.linspace(t_points[0], t_points[-1], 1600)
...
def main():
    print("start!")
    ox, oy = generate_track('/home/ansh/MotionPlanning/HybridAstarPlanner/waypoints.csv', track_width=3.1)
    gx, gy, gyaw0 = 0, 8, np.deg2rad(0.0)
