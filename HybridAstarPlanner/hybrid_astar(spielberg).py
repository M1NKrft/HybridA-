...
class C:  # Parameter config
    ...
    COLLISION_CHECK_STEP = 1  # skip number for collision check
    ...
    RF = 4.5 * SCALE_FACTOR  # [m] distance from rear to vehicle front end of vehicle
    RB = 1.0 * SCALE_FACTOR  # [m] distance from rear to vehicle back end of vehicle
    W = 3.0 * SCALE_FACTOR  # [m] width of vehicle
    WD = 0.7 * W * SCALE_FACTOR  # [m] distance between left-right wheels
    WB = 3.5* SCALE_FACTOR   # [m] Wheel base
    TR = 0.5 * SCALE_FACTOR  # [m] Tyre radius
    TW = 1 * SCALE_FACTOR  # [m] Tyre width
    MAX_STEER = 0.6* SCALE_FACTOR   # [rad] maximum steering angle
...
def generate_track(csv_file: str, track_width: float):
    ...
    spline_x = CubicSpline(t_points, x_points, bc_type='not-a-knot')
    spline_y = CubicSpline(t_points, y_points, bc_type='not-a-knot')
    ...
    left_x = x_fine - (track_width / 2) * dy_unit
    left_y = y_fine + (track_width / 2) * dx_unit
    right_x = x_fine + (track_width / 2) * dy_unit
    right_y = y_fine - (track_width / 2) * dx_unit

    return [*left_x, *right_x], [*left_y, *right_y]

    # plt.figure(figsize=(10, 8))
    # plt.plot(x_fine, y_fine, label='Centerline Spline', color='blue')
    # plt.axis('equal')  # Ensure equal scaling on both axes
    # plt.plot(left_x, left_y, label='Left Boundary', linestyle='--', color='red')
    # plt.plot(right_x, right_y, label='Right Boundary', linestyle='--', color='green')
    # plt.show()

def main():
    print("start!")
    ox, oy = generate_track('/home/ansh/MotionPlanning/HybridAstarPlanner/Spielberg_centerline.csv', track_width=3.0)
    gx, gy, gyaw0 = -51, 16, np.deg2rad(0.0)

    # image = cv2.imread('Spielberg_map.png', cv2.IMREAD_GRAYSCALE)
    ...
    sx, sy, syaw0 = 0, 0, np.deg2rad(0.0)
    ...
    # ox = x_indices.tolist()
    # oy = (binary_image.shape[0] - 1 - y_indices).tolist()  # Invert y to make origin bottom-left:

    # print("X coordinates of obstacles:", ox)
    # print("Y coordinates of obstacles:", oy)
    print("ch1")
    import matplotlib.pyplot as plt
    plt.plot(ox,oy, "o")
    plt.plot(sx,sy,".")
    ...
    for k in range(len(x)):
        plt.cla()
        plt.plot(ox, oy, ".")
        plt.plot(x, y, linewidth=1.05, color='r')
